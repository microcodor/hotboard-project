...
async function main() {
  ...
  for (const [key, result] of Object.entries(results)) {
    ...
    console.log(`${status} ${platform.name}: ${result.success ? `${result.count}条数据, ${result.duration}ms` : result.error}`);
  ...
  console.log('\n📊 统计:');
  console.log(`   成功: ${success}/${success + failed}`);
  console.log(`   失败: ${failed}/${success + failed}`);
}
